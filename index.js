    ...
    rl.question('Enter your choice: ', (choice) => {
        switch (choice) {
            ...
            case '3':
                ...
                fs.readdirSync('./plugins').forEach(function(file) {
                    ...
                    try {
                        ...
                        plugin.logic('http://example.com', '<html></html>', {'hello': 'world'}, function(data) {})
            ...
            case '5':
                ...
                db.get("SELECT COUNT(*) as count FROM scanned WHERE scraped = 1", (err, row) => {
                    scrapeDone = row.count
                    db.get("SELECT COUNT(*) as count FROM scanned WHERE scraped = 0 AND httpCode = '200'", (err, row) => {
                        scrapeQueue = row.count
                        db.get("SELECT COUNT(*) as count FROM scanned", (err, row) => {
                            crawlDone = row.count
                            db.get("SELECT COUNT(*) as count FROM queue WHERE type = 'crawl'", (err, row) => {
                                crawlQueue = row.count
                                db.get("SELECT COUNT(*) as count FROM loot", (err, row) => {
